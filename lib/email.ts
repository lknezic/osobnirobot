...
export function welcomeEmail(name: string): { subject: string; html: string } {
  ...
  return {
    subject: `${safeName} is hired — your AI worker is live`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px 24px; background: #0a0a0a; color: #e5e5e5;">
        ...
        <p style="color: #888; font-size: 14px; line-height: 1.6; margin-bottom: 24px;">
          <strong>${safeName}</strong> is now live and working for you 24/7. Head to your dashboard to watch it in action.
        </p>
...
export function trialExpiringEmail(name: string, daysLeft: number): { subject: string; html: string } {
  const safeName = escapeHtml(name);
  return {
    subject: `${safeName}'s trial ends in ${daysLeft} day${daysLeft !== 1 ? "s" : ""}`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px 24px; background: #0a0a0a; color: #e5e5e5;">
        <h1 style="font-size: 22px; color: #fff; margin-bottom: 8px;">Your trial is ending soon</h1>
        <p style="color: #888; font-size: 14px; line-height: 1.6; margin-bottom: 24px;">
          <strong>${safeName}</strong>'s free trial ends in <strong>${daysLeft} day${daysLeft !== 1 ? "s" : ""}</strong>.
          After that, your worker will stop running unless you subscribe.
        </p>
        <a href="https://instantworker.ai/dashboard" style="display: inline-block; padding: 12px 24px; background: linear-gradient(135deg, #7c6bf0, #9b7bf7); color: #fff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 14px;">Keep ${safeName} running →</a>
...
export function paymentFailedEmail(name: string): { subject: string; html: string } {
  const safeName = escapeHtml(name);
  return {
    subject: `Payment failed — ${safeName} will stop soon`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px 24px; background: #0a0a0a; color: #e5e5e5;">
        <h1 style="font-size: 22px; color: #fff; margin-bottom: 8px;">Payment failed</h1>
        <p style="color: #888; font-size: 14px; line-height: 1.6; margin-bottom: 24px;">
          We couldn't process your payment for <strong>${safeName}</strong>. Please update your payment method to keep your worker running.
        </p>
